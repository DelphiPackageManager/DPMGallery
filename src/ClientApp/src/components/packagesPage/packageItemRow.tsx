          ...
          <div className="flex flex-row w-full items-baseline mb-1">
            ...
            <div className="ml-3 text-sm md:text-base text-gray-400">
              ...
              {pkg.owners.map((owner, index) => {
                return (
                  <NavLink className="ml-1 text-sky-600 dark:text-sky-600 hover:underline" key={index} to={`/profiles/${owner}`}>
                    {owner}
                  </NavLink>
                );
              })}
          ...
          <div className="flex flex-row flex-wrap text-gray-400 dark:text-gray-500 mb-2 items-start text-sm">
            ...
            <div className="flex flex-row items-center">
              ...
              {pkg.tags?.map((tag, index) => {
                return (
                  <NavLink className="mr-1 text-sky-700 dark:text-sky-700 hover:underline" key={index} to={`/packages?q=tag%3A${tag}`}>
                    {tag}
                  </NavLink>
                );
              })}
